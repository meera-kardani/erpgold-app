    ...
    if doc.docstatus == 1: 
        ...
        for item in doc.items:
            stock_ledger_entries = frappe.get_all("Stock Ledger Entry", 
                                                  filters={"voucher_no": doc.name, "item_code": item.item_code},
            ...
            for entry in stock_ledger_entries:
                gold_ledger_entry = frappe.get_doc({
                    ...
                    "debit_amount": item.custom_total_amount if doc.doctype == "Delivery Note" else 0,
                    "credit_amount": item.custom_total_amount if doc.doctype == "Purchase Receipt" else 0,
                    "debit_gold": item.custom_fine_weight if doc.doctype == "Delivery Note" else 0,
                    "credit_gold": item.custom_fine_weight if doc.doctype == "Purchase Receipt" else 0,
